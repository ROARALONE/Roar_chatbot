    ...
    for c in set(categorycodes):
        ...
        for key in set(considering):
            ...
            try:
                ...
                answer_from_bot = {"fulfillmentText": f"{response_place}"}
                answer_from_bot = json.dumps(answer_from_bot, indent=4)
                return answer_from_bot
                for i in range(0,len(response_place["result"])):
                    ...
                    if len(title) > 45:
                        title = response_place['result'][i]['place_name'][:30] + "..."

                    col = {"actions":[],"defaultAction":{},"imageBackgroundColor": "#FFFFFF","title": title,
                              "thumbnailImageUrl":image,
                              "text": c}

                    uri = f"http://127.0.0.1:5000/{c}/{response_place['result'][i]['place_id']}"
                    col["actions"].append({
    ...
    try:
        ...
        for i in range(len(response_trip['result'])):
            ...
            if len(title_trip) > 45:
                title_trip = response_trip['result'][i]['route_name'][:30] + "..."
            ...
            uri = f"http://127.0.0.1:5000/Route/{response_trip['result'][i]['route_id']}"
